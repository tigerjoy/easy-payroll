...
- Add a new staff member via a multi-step wizard (Basic Info → Role → Documents → Salary → Custom Fields). **Start Date** is required to initialize attendance tracking.
- View a staff member's full profile on a dedicated detail page, including their current **Holiday Balance**.
...
- Card grid displays photo, name, role, **Holiday Balance**, and key contact info
- Table view shows columns for name, role, phone, start date, **Holiday Balance**, status
