...
import type { StaffDirectoryProps, EmploymentType } from '@/../product/sections/staff-directory/types'
...
export function StaffDirectory({
  ...
  const roles = useMemo(() => {
    ...
    employees.forEach(emp => {
      roleSet.add(emp.employment.role)
    })
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(emp => {
      ...
      if (searchQuery) {
        ...
        const roleMatch = emp.employment.role.toLowerCase().includes(query)
      ...
      // Status filter
      if (statusFilter !== 'all' && emp.employment.status !== statusFilter) return false

      // Role filter
      if (roleFilter && emp.employment.role !== roleFilter) return false

      // Employment type filter
      if (employmentTypeFilter !== 'all' && emp.employment.employmentType !== employmentTypeFilter) return false
  ...
  const handleArchive = (id: string) => {
    ...
    if (employee?.employment.status === 'archived') {
      onRestore?.(id)
    } else {
      onArchive?.(id)
    }
